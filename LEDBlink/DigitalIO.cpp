...
namespace Driver{
	namespace GPIO{
		...
		const Digital& Digital::EnableInt(const uint32_t _ch, const IntEdge _edge, const CallBackRef _callback){
			IntCh = _ch;
		...
		const Digital& Digital::EnableInt(const uint32_t _ch, const IntEdge _edge, const CallBackRRef _callback){
			IntCh = _ch;
